	...
	fn process_message(&mut self, message: DocumentMessage, responses: &mut VecDeque<Message>, context: DocumentMessageContext) {
		...
		match message {
			...
			DocumentMessage::NudgeSelectedLayers {
				...
				let document_to_viewport = self.navigation_handler.calculate_offset_transform(viewport.logical_center().into(), &self.document_ptz);
			...
			DocumentMessage::PasteImage {
				...
				let viewport_location = mouse.map_or(viewport.logical_center().into_dvec2() + viewport.logical_offset().into_dvec2(), |pos| pos.into());
			...
			DocumentMessage::RenderRulers => {
				...
				let document_to_viewport = self.navigation_handler.calculate_offset_transform(viewport.logical_center().into(), current_ptz);
			...
			DocumentMessage::PTZUpdate => {
				if !self.graph_view_overlay_open {
					let transform = self.navigation_handler.calculate_offset_transform(viewport.logical_center().into(), &self.document_ptz);
					...
					let transform = self
						.navigation_handler
						.calculate_offset_transform(viewport.logical_center().into(), &network_metadata.persistent_metadata.navigation_metadata.node_graph_ptz);
